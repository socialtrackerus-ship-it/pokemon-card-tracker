...
export function Header() {
  ...
  // Public-first navigation — discovery, not dashboard
  const publicNav = [
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-[var(--surface-0)]/90 backdrop-blur-xl">
      <div className="container flex h-12 items-center gap-5">
        {/* Brand */}
          ...
          {publicNav.map((link) => {
            ...
            return (
              <Link key={link.href} href={link.href}
                ...
                <span className={active ? 'text-[var(--text-primary)]' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}>
                  {link.label}
                </span>
                {active && <span className="absolute bottom-0 left-2.5 right-2.5 h-[2px] rounded-full bg-[var(--brand)]" />}
              </Link>
            )
          })}
        </nav>
        ...
        <Link href="/sets?focus=search"
          ...
          <kbd className="ml-3 px-1.5 py-0.5 text-[10px] bg-[var(--surface-1)] border border-[var(--border-default)] rounded font-mono">/</kbd>
        </Link>
          ...
          {session ? (
            <div className="relative" ref={userMenuRef}>
              <button onClick={() => setUserMenuOpen(!userMenuOpen)}
                className="flex items-center gap-1.5 p-1 rounded-lg hover:bg-[var(--surface-hover)] transition-colors">
              ...
              {userMenuOpen && (
                <div className="absolute right-0 top-full mt-2 w-48 surface-1 rounded-lg overflow-hidden animate-fade z-50">
                  <div className="px-3 py-2.5 border-b border-[var(--border-subtle)]">
                    <p className="text-[12px] font-medium truncate">{session.user?.email}</p>
                  </div>
                  <div className="py-1">
            ...
            <Link href="/auth/login"
              className="text-[12px] font-medium text-[var(--text-secondary)] hover:text-[var(--text-primary)] px-2.5 py-1.5 rounded-lg hover:bg-[var(--surface-hover)] transition-colors">
              Sign in
            </Link>
          )}

          {/* Mobile hamburger */}
          <button className="md:hidden p-2 rounded-lg hover:bg-[var(--surface-hover)] transition-colors text-[var(--text-secondary)]"
              ...
              return (
                <Link key={link.href} href={link.href} onClick={() => setMobileOpen(false)}
                  className={`px-3 py-2.5 text-[13px] font-medium rounded-lg transition-colors ${
                    active ? 'text-[var(--text-primary)] bg-[var(--surface-hover)]' : 'text-[var(--text-secondary)]'
                  }`}>
                  {link.label}
                </Link>
              )
            ...
            {session && (
              <>
                ...
                <Link href="/collection" onClick={() => setMobileOpen(false)}
                  className="px-3 py-2.5 text-[13px] font-medium text-[var(--text-secondary)] rounded-lg">
                  My Collection
                </Link>
